...
def fix_module1():
    ...
    # 查找并替换数组定义
    # 查找注释位置
    comment_pattern = r'// 基于Excel文件实际内容的完整\d+行产品目录'
    comment_match = re.search(comment_pattern, content)
    
    if comment_match:
        comment_start = comment_match.start()
        # 查找const fullExcelData的位置
        array_start_pattern = r'const fullExcelData = \[.*?\];'
        array_match = re.search(array_start_pattern, content[comment_start:], re.DOTALL)
        
        if array_match:
            array_start = comment_start + array_match.start()
            array_end = comment_start + array_match.end()
