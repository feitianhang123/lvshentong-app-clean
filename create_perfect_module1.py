...
def create_perfect_module1():
    ...
    # 保存新文件
    output_path = 'android-project/app/src/main/assets/www/module1_fixed_final_v10.html'
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print(f'已生成模块一v10文件: {output_path}')
    ...
    if valid_rows == len(excel_data):
        ...
        # 调试：显示前几行
        print('前5行:')
        for i in range(min(5, len(excel_data))):
            print(f'Excel行{i}: {excel_data[i]}')
        
        print('JS数组前5行:')
        js_lines = js_array.split('\n')
        for i in range(min(10, len(js_lines))):
            print(f'JS行{i}: {js_lines[i]}')
    
    return output_path

if __name__ == "__main__":
    output_file = create_perfect_module1()
    ...
    if output_file:
        ...
        # 替换模块一链接
        index_content = index_content.replace('module1_fixed_final_v6.html', 'module1_fixed_final_v10.html')
